...
public class EX3_LM_2_1 {
    ...
    public static class BorrowRecord {
        ...
        public BorrowRecord(String studentID, String bookName, String borrowDate) {
            this.studentID = studentID;
            this.bookName = bookName;
        ...
        public String toFileString() {
            return studentID + "," + bookName + "," + borrowDate;
        }

        public void display() {
            System.out.println("Borrowed Book: \"" + bookName + "\" by Student ID: " + studentID + " on " + borrowDate);
        }
    }
    ...
    public static void saveBorrowRecordsToFile(BorrowRecord[] records, String filename) {
        try {
            FileWriter writer = new FileWriter(filename);
            ...
            System.out.println("Error writing borrow records: " + e.getMessage());
        }
    }
    ...
    public static BorrowRecord[] readBorrowRecordsFromFile(String filename, int size) {
        BorrowRecord[] records = new BorrowRecord[size];
        try {
            BufferedReader reader = new BufferedReader(new FileReader(filename));
            String line;
            int index = 0;
            while ((line = reader.readLine()) != null && index < size) {
                String[] parts = line.split(",");
                records[index++] = new BorrowRecord(parts[0], parts[1], parts[2]);
            }
            reader.close();
        } catch (IOException e) {
    ...
    public static void main(String[] args) {
        ...
        BorrowRecord[] borrowRecords = {
                new BorrowRecord("403001253", "book 1", "2025-04-12"),
                new BorrowRecord("402502225", "book 3", "2025-04-11")
        };

        saveBooksToFile(books, "books.txt");
        saveStudentsToFile(students, "students.txt");
        saveBorrowRecordsToFile(borrowRecords, "borrowRecords.txt");

        Book[] loadedBooks = readBooksFromFile("books.txt", 4);
        Student[] loadedStudents = readStudentsFromFile("students.txt", 3);
        BorrowRecord[] loadedBorrowRecords = readBorrowRecordsFromFile("borrowRecords.txt", 2);
        ...
        System.out.println("\nLoaded borrow records:");
        for (BorrowRecord br : loadedBorrowRecords) {
            br.display();
        }
    }
}
